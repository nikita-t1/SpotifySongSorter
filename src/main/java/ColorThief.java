...
public class ColorThief {
     ...
     */
    public static List<Color> getColor(BufferedImage sourceImage) {
        int[][] palette = getPalette(sourceImage, 5);
        if (palette == null) {
            return null;
        }
        List<Color> colorList = new ArrayList<>();
        for (int[] ints : palette) {
            var red = padStart(Integer.toHexString(ints[0]));
            var green = padStart(Integer.toHexString(ints[1]));
            var blue = padStart(Integer.toHexString(ints[2]));
     ...
     */
    public static List<Color> getColor(BufferedImage sourceImage, int quality, boolean ignoreWhite) {
